...
drop table Temp_Fact_Hum

create table Temp_Fact_Hum
(
	Su_Plant_ID int,
	Su_Profile_ID int,
	Su_User_ID int,
...
DELETE FROM Temp_Fact_Hum;

insert into Temp_Fact_Hum (Plant_ID, Profile_ID, [Date] , [User_ID] , [Time], [Sensor_Value] , Hum_Status)
...
--------------------------------UPDATING THE TEMP FACT Humedity WITH SURROGATE KEY FROM THE DW-------------------------------------

UPDATE Temp_Fact_Hum
	SET Su_User_ID = (
	SELECT TOP 1 Su_User_ID
	FROM Dim_SEP4_PMI.dbo.Dim_Users u
	WHERE u.[User_ID] = Temp_Fact_Hum.[User_ID]
...
UPDATE Temp_Fact_Hum
	...
	WHERE p.[Plant_ID] = Temp_Fact_Hum.[Plant_ID]
...
UPDATE Temp_Fact_Hum
	...
	WHERE pp.[Profile_ID] = Temp_Fact_Hum.[Profile_ID]
...
UPDATE Temp_Fact_Hum
	...
	WHERE d.[CalendarDate] = Temp_Fact_Hum.[Date]
	);
...
UPDATE Temp_Fact_Hum
	...
	WHERE t.[Time] = Temp_Fact_Hum.[Time]
	);

SELECT * FROM Temp_Fact_Hum;
-----------------------------POPULATING THE CO2 FACT IN THE DW WITH THE NEW ROWS---------------------------

INSERT INTO Dim_SEP4_PMI.dbo.Dim_Fact_Hum
    (Su_Plant_ID, Su_Profile_ID, Su_Date_ID, Su_Time_ID, Su_User_ID, Sensor_Value, Hum_Status)
    SELECT Su_Plant_ID ,Su_Profile_ID, Su_Date_ID, Su_Time_ID, Su_User_ID, Sensor_Value, Hum_Status FROM Temp_Fact_Hum;
